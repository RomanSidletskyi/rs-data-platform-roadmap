...
class PipelineSmokeTest(unittest.TestCase):
    def test_pipeline_builds_curated_output(self) -> None:
        ...
        result = subprocess.run(
            [str(PYTHON_EXECUTABLE), "run_pipeline.py"],
            cwd=PROJECT_ROOT,
            check=True,
            capture_output=True,
            text=True,
        ...
        with sqlite3.connect(warehouse_path) as connection:
            ...
            electronics_row = connection.execute(
                """
                SELECT gross_revenue
                FROM curated_daily_sales
                WHERE sales_date = '2026-03-20' AND product_category = 'electronics'
                """
            ).fetchone()

        self.assertEqual(row_count, 8)
        self.assertIsNotNone(electronics_row)
        self.assertEqual(electronics_row[0], 249.99)
