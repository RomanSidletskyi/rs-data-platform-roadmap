...
ROOT="01-python/pet-projects"

mkdir -p \
  "$ROOT/01_api_to_csv_pipeline" \
  "$ROOT/02_json_normalizer" \
  "$ROOT/03_log_parser_pipeline" \
  "$ROOT/04_data_quality_checker" \
  "$ROOT/05_incremental_ingestion_simulator"

cat << 'EOF' > "$ROOT/01_api_to_csv_pipeline/README.md"
# API to CSV Pipeline
...
cat << 'EOF' > "$ROOT/02_json_normalizer/README.md"
# JSON Normalizer
...
cat << 'EOF' > "$ROOT/03_log_parser_pipeline/README.md"
# Log Parser Pipeline
...
cat << 'EOF' > "$ROOT/04_data_quality_checker/README.md"
...
cat << 'EOF' > "$ROOT/05_incremental_ingestion_simulator/README.md"
...
echo "All Python pet-project README files were created successfully."