...
Where a separate ready implementation is useful, it should live in a sibling folder named `reference_example_<project-name>`.

That keeps the learning path consistent:

- guided project first
- reference example second
- selective use of reference examples only for the strongest representative cases
...
Current dbt shared target structure:

- `shared/configs/templates/dbt/` for `profiles`, `dbt_project`, `selectors`, and GitHub Actions examples
- `shared/templates/dbt/macros/` for reusable macros such as `generate_schema_name.sql`
- `shared/scripts/helpers/dbt/` for small helper scripts used by more than one dbt project
