...
## 9. Production pipeline shape affects scaling

Airflow performance is heavily influenced by pipeline shape, not only infrastructure size.
...
- validate before publishing downstream outputs

### Bad strategy

- one giant task with many responsibilities

### Why bad is bad

- retries are expensive
- blast radius is large
- queue time and worker occupancy increase

---

## 10. Configuration and environment separation

Scaling problems often become operational problems when environments are not separated cleanly.

Good strategy:

- same DAG code across environments
- different config, secrets, pools, and connections per environment

Bad strategy:

- different logic for dev, stage, and prod
- shared unsafe defaults across environments

### Why bad is bad

- debugging becomes inconsistent
- incidents spread across environments
- cost and behavior become harder to predict
...
## 13. Good vs bad scaling strategies
