...
## 3. Scheduling semantics and logical intervals

Airflow scheduling is not only about clock time.

It is about creating DAG runs for logical data intervals.

Example:

- a daily DAG run created on `2024-01-02`
- may actually process data for `2024-01-01`

Good strategy:

- use `data_interval_start`  
- use `data_interval_end`  
- design runtime logic around intervals  

Bad strategy:

- use `datetime.now()` for extraction or output partition logic  

Why bad is bad:

- reruns and backfills become inconsistent  
- a task may process the wrong slice of data  

--------------------------------------------------

## 4. Catchup, backfill, and rerun reality

### Catchup

Catchup controls whether Airflow creates historical DAG runs for past intervals.

### Backfill

Backfill means reprocessing historical intervals intentionally.

### Why this matters for runtime design

The runtime model is only correct if tasks can safely handle:

- historical runs  
- retries  
- reruns for the same interval  

Good strategy:

- write interval-scoped outputs  
- make tasks idempotent  
- use deterministic interval-based reads and writes  

Bad strategy:

- assume only one forward-moving execution path  

Production failure scenario:

- backfill is triggered for an old interval  
- task uses current time and append-only writes  
- historical output becomes corrupted or duplicated  

--------------------------------------------------

## 5. Executors (deep explanation)
...
## 11. State updates and consistency
