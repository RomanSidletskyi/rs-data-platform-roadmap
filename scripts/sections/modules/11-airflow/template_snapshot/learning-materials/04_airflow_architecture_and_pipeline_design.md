...
Tasks should be safe to run more than once for the same interval.

Good strategy:

- overwrite a deterministic target  
- merge by business key  
- write partition-scoped outputs  
...
The same inputs for the same interval should produce the same outputs.

Bad examples:

- random branching logic  
- wall-clock-based extraction windows  
- hidden dependencies outside the declared workflow  

### Production scenario

A retry for the same interval creates different output than the first run:

- downstream tasks consume conflicting results  
- backfill behavior becomes untrustworthy  
- incident analysis becomes difficult  
...
Airflow should coordinate workflow state, not move large datasets between tasks.

Good strategy:

- use XCom for small metadata such as file paths, IDs, or row counts  
- use storage systems or databases for actual data exchange  

Bad strategy:

- pass DataFrames or large payloads through XCom  

### Why bad is bad

- metadata DB grows too fast  
- scheduler and UI slow down  
- Airflow becomes a bottleneck instead of an orchestrator  

### Example

    # good
    xcom_push(key="raw_path", value="s3://bucket/orders/date=2024-01-01/")

    # bad
    xcom_push(key="orders", value=large_dataframe)

--------------------------------------------------

## 11. Airflow vs processing layer boundary
