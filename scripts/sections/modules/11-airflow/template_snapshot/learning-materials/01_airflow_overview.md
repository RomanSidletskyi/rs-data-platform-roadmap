...
Thousands of trivial DAGs:
    ...
    ### Core concepts you must internalize early

    Airflow becomes much easier to use correctly once a few concepts are explicit from the start.
    ...
    A DAG is not the same thing as runtime execution.
    ...
    - one task  
    - in one specific DAG run  
    - for one specific logical interval  

    This matters because retries, reruns, and failures all happen at the task-instance level, not at the abstract task-definition level.

    #### DAG Run

    A DAG Run represents one execution of the DAG for a specific interval or trigger event.

    When reasoning about Airflow, it is important to separate:

    - DAG definition  
    - DAG run  
    - task instance  

    If these concepts are mixed together mentally, debugging becomes much harder.
    ...
    In practice this means a daily DAG may run on one date while processing data for the previous interval.

    Good strategy:

    - use `data_interval_start`  
    - use `data_interval_end`  
    - make task logic interval-aware  

    Bad strategy:

    - use `datetime.now()` as the business time boundary  

    Why bad is bad:

    - reruns become inconsistent  
    - backfills produce incorrect results  
    - different runs of the same interval produce different data  
    ...
    - a backfill runs for an old day  
    - task logic uses current time instead of interval time  
    - wrong data is extracted or published  
