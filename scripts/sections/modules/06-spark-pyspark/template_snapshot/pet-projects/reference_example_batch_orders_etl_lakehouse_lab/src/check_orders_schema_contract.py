...
def check_file(path: str) -> list[str]:
    ...
    with open(path, "r", encoding="utf-8") as handle:
        for line_number, raw_line in enumerate(handle, start=1):
            ...
            if "country_code" in row and row["country_code"] != row["country_code"].strip().upper():
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("Usage: python3 src/check_orders_schema_contract.py <jsonl-path>")
    findings = check_file(sys.argv[1])
    if findings:
        sys.stdout.write("\n".join(findings) + "\n")
        return
    sys.stdout.write("schema-check=pass\n")
